...
compute_covariance_S <- function(ds, formula, structure = "combined",var_beta_star = NULL, data = NULL, beta = NULL,ds2=NULL, n1=NULL,n2=NULL) {
  weights_S <- weights(ds)
  
  if (structure == "combined") {
    if(is.null(ds2)||is.null(n1)||is.null(n2)){
    ...
    u_S <- (ds$variables$y - p_hat_S)[(n1 + 1):(n1 + n2)] * X_S[(n1 + 1):(n1 + n2), ]  
    ...
    var_S <- (n2^2 / (n1 + n2)^2)*vcov(svytotal(u_formula, ds2))
...
compute_covariance_s <- function(ds, formula_i, structure,wt0,u_star,ds1=NULL,n1=NULL,n2=NULL) {
  ...
  u <- (ds$variables$y - p_hat) * X  # Influence function for model
...
custom_variance <- function(design, formula) {
  ...
  u <- (design$variables$y - p_hat) * X  # Influence function (score function)
...
calibrate_beta_variance <- function(ds_s,ds_S,formula,formula_i,u_star,wt0,structure = "combined",var_beta_star = NULL,data=NULL,beta=NULL,ds2=NULL,n1=NULL,n2=NULL,ds1=NULL){
  ...
  # Calculate V(S)
  var_S1 <- compute_covariance_S(ds_S,formula,structure,var_beta_star,data,beta,ds2,n1,n2)
